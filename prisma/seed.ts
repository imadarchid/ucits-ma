...
const categories = [{name: 'Actions'}, {name: 'OMLT'}, {name: 'Monétaire'}, {name: 'Diversifié'}, {name: 'OCT'}, {name: 'Contractuel'}]
...
const catHandler = (value: string) => {
    return categories.findIndex((x) => { return x.name === value}) + 1
}
...
const load = async (file: string) => {
    let xlsxData = await fs.readFile(path.resolve(__dirname, file));
    ...
    for (let i = 0; i < funds.length; i++) {
        ...
        await prisma.rates.upsert({
            ...
            create: {
                ...
                mgt_fee: ((funds[i]['Frais de gestion'] === '-') ? 0 : funds[i]['Frais de gestion']), // data source inconsistency
