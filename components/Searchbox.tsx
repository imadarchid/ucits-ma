...
interface SearchBoxProps {
  setFund: (isin_code: string) => void
}

export const Searchbox = ({setFund}: SearchBoxProps) => {
  const [searchText, setSearchText] = useState("");

  const funds = trpc.funds.findFunds.useQuery({ name: searchText });

  const handleSelect = (isin_code: string) => {
    setSearchText(isin_code)
    setFund(isin_code)
  };
        ...
        <div className="min-w-full relative">
          {searchText.length ? (
            ...
            <div className="absolute min-w-full bg-white flex flex-col divide-x border-solid border-2 rounded-lg">
              {funds.data && funds.data?.map(x => (
                <div key={x.isin_code} className="min-w-full flex items-center shadow-sm px-4 py-2 hover:bg-slate-100 cursor-pointer selection:invisible" onClick={() => handleSelect(x.isin_code)}>
                    <p><b>{x.isin_code}</b> | {x.name}</p>
                </div>
              ))}
            </div>
            </>
